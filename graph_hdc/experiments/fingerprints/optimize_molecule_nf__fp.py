"""
Fingerprint-based Molecular Optimization with Normalizing Flow Constraints

This experiment extends the base optimize_molecule_nf.py experiment to use
...
descent with normalizing flow constraints to reach target property values.
...
Key Features:
    ...
    - Normalizing flow constraint for in-distribution optimization
    - No PCA (works in original representation space)

Design Considerations:
    While fingerprints are discrete by nature, the gradient-based optimization
    operates in a continuous relaxation of the fingerprint space. The normalizing
    flow helps constrain the optimization to stay in regions that are similar to
    the training distribution. The optimized representations may not correspond
    to valid molecular structures, but the closest matching molecules in the test
    set provide a proxy for achievable targets.
...
Example:
    .. code-block:: bash
        ...
        # Run with debug mode
        python optimize_molecule_nf__fp.py

        # Run with configuration
        python -m pycomex run optimize_molecule_nf__fp__clogp.yml
"""
import numpy as np
...
#       and may provide smoother optimization landscapes. For normalizing flow
#       constraints, larger sizes provide more capacity for modeling the distribution.
...
def process_dataset(e: Experiment,
    ...
    in a continuous relaxation of the fingerprint space. The normalizing flow
    constraint helps keep the optimization within reasonable bounds by penalizing
    representations that are unlikely under the training distribution.

    Optimization Considerations:
    - The optimization will modify fingerprint bits continuously (0 to 1)
    - Normalizing flow provides likelihood constraint to stay in-distribution

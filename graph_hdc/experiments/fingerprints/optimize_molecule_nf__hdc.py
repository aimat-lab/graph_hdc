"""
HDC-based Molecular Optimization with Normalizing Flow Constraints
...
molecular graphs into high-dimensional hypervectors, which are then optimized
via gradient descent with normalizing flow constraints to reach target property values.

Key Features:
    - Continuous FHRR encodings for better regression performance
    - Message passing with configurable depth
    - High-dimensional representations (default 4096D)
...
Example:
    .. code-block:: bash
        ...
        # Run with debug mode
        python optimize_molecule_nf__hdc.py
...
#       but increase computational cost. For optimization with normalizing flows,
#       larger sizes provide smoother gradients and better flow modeling.
...
#       performs better for regression tasks and provides smoother optimization landscapes.
...
def process_dataset(e: Experiment,
    ...
    particularly well-suited for gradient-based optimization with flow constraints due to:
    ...
    4. High dimensionality allows for effective normalizing flow modeling
    ...
    if e.ENCODING_MODE == 'continuous':
        ...
        # May result in less smooth optimization landscapes
    ...
    def process_dataset_cached():
        ...
        multiple optimization experiments on the same dataset with the same
        encoding parameters.
